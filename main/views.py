...
import os
os.environ['OPENAI_API_KEY'] = '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'

gpt_client = OpenAI(api_key= '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')
...
class Workout(APIView):
    ...
    def post(self, request):
        ...
        storage_context = StorageContext.from_defaults(persist_dir='/midjourney-back/media/train/done2')
        index = load_index_from_storage(storage_context)
        response = index.as_query_engine().query(f"Create a workout plan in JSON format as a list of dictionaries. Include exactly 6 exercises targeting the {request.data["main"]}. Take into account any {request.data["damage"]} that requires modification or avoidance. Tailor the exercises to the specified workout {request.data["level"]} and use only the available {request.data["equipment"]} in the required environment {request.data["location"]}. Structure the routine to meet the specified goal {request.data["goal"]}, and include sets, reps, and rest recommendations for optimal results.Each exercise should be represented as a dictionary with fields: gif_url, exercise_name, details, sets, and repeats.")
        return Response(response.response) 
...
class Workout2(APIView):
    ...
    def post(self, request):
        ...
        storage_context = StorageContext.from_defaults(persist_dir='/midjourney-back/media/train/done2')
        index = load_index_from_storage(storage_context)
        response = index.as_query_engine().query(f"Create a workout plan in JSON format as a list of dictionaries. Include exactly 2 exercises targeting the {request.data["secondary"]}. Take into account any {request.data["damage"]} that requires modification or avoidance. Tailor the exercises to the specified workout {request.data["level"]} and use only the available {request.data["equipment"]} in the required environment {request.data["location"]}. Structure the routine to meet the specified goal {request.data["goal"]}, and include sets, reps, and rest recommendations for optimal results.Each exercise should be represented as a dictionary with fields: gif_url, exercise_name, details, sets, and repeats.")
        return Response(response.response) 
