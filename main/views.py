...
class Workout(APIView):
    ...
    def post(self, request):
        ...
        # documents = SimpleDirectoryReader("/midjourney-back/files").load_data()
        # index = GPTVectorStoreIndex.from_documents(documents)

        # index.storage_context.persist(persist_dir="/midjourney-back/media/train/done2")
        storage_context = StorageContext.from_defaults(
            persist_dir="/midjourney-back/media/train/done2"
        )
        index = load_index_from_storage(storage_context)
        response = index.as_query_engine().query(f"Create a workout plan in JSON format with two lists: one named main for the Primary Muscle exercises and one named second for the Secondary Muscle exercises. Include exactly 6 unique exercises in the {request.data["main"]} in the main list and 2 unique exercises in the {request.data["secondary"]} in the second list with no repeated exercises across or within lists. Take into account any {request.data["damage"]} that requires modification or avoidance. Tailor the exercises to the specified workout {request.data["level"]} and use only the available {request.data["equipment"]} in the required environment {request.data["location"]}. Structure the routine to meet the specified goal {request.data["goal"]}, and include sets, reps, and rest recommendations for optimal results.Each exercise should be represented as a dictionary with fields: gif_url, exercise_name, exercise_description, sets, and repeat in persian.")
...
#         )
#         index = load_index_from_storage(storage_context)
#         response = index.as_query_engine().query(f"Create a workout plan in JSON format with two lists: one named main for the Primary Muscle exercises and one named second for the Secondary Muscle exercises. Include exactly 6 exercises in the {request.data["main"]} in the main list and 2 exercises in the {request.data["secondary"]} in the second list. Take into account any {request.data["damage"]} that requires modification or avoidance. Tailor the exercises to the specified workout {request.data["level"]} and use only the available {request.data["equipment"]} in the required environment {request.data["location"]}. Structure the routine to meet the specified goal {request.data["goal"]}, and include sets, reps, and rest recommendations for optimal results.Each exercise should be represented as a dictionary with fields: gif_url, exercise_name, exercise_description, sets, and repeats.")        response = response.response
#         response = response.replace("```json", "").replace("```", "")
#         response = json.loads(response)
#         return Response(response) 
